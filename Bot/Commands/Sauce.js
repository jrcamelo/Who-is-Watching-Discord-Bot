...
class SauceCommand extends BaseCommand {
  ...
  async execute() {
    ...
    if (!(await this.sauce.setImage())) {
      return this.reply({ content: "Could not find an image." });
    }
    if (!(await this.sauce.search())) {
      return this.reply({ content: "Could not find sauce. Maybe there was an error or we hit the 6 searches per 30 seconds limit." })
    }
